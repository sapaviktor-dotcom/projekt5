# Проект "Мои банковские операции"

## Описание:

Проект "Мои банковские операции" - это виджет на Python, 
который показывает несколько последних успешных банковских операций клиента.


## Установка:

+ Установите на свой компьютер GitHub.
+ Клонируйте репозиторий с GitHub  с помощью веб-URL.
```
 git clone https://github.com/sapaviktor-dotcom/projekt5.git
```


## Использование:

В текущем виде, программа запускается в main.py



## Примеры использования функций:

+ Модуль masks
содержит функции для наложения масок на конфиденциальные данные:

**get_mask_card_number**

Принимает на вход номер карты в виде числа и возвращает маску номера по правилу XXXX XX** **** XXXX 

     Пример: 7000792289606361 → 7000 79** **** 6361

**get_mask_account**

Принимает на вход номер счета и возвращает маску номера по правилу **XXXX

     Пример: 73654108430135874305 → **4305

+ Модуль widget
содержит функции для работы с банковскими данными:

**mask_account_card**

Принимает строку, содержащую тип и номер карты или счета, и возвращает строку с замаскированным номером.

     Пример: "Visa Platinum 8990922113665229" → "Visa Platinum  8990 92** **** 5229"

**get_date**

Принимает на вход строку с датой в формате "2024-03-11T02:26:18.671407"
и возвращает строку с датой в формате "ДД.ММ.ГГГГ" ("11.03.2024").

      Пример: "2024-03-11T02:26:18.671407" → "11.03.2024"

+ Модуль processing
содержит функции для фильтрации и сортировки банковских операций:

**filter_by_state**

Фильтрует список операций по статусу выполнения (например по 'EXECUTED')

      Пример: [
               {'id': 41428829, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364'},
               {'id': 939719570, 'state': 'EXECUTED', 'date': '2018-06-30T02:08:58.425572'},
               {'id': 594226727, 'state': 'CANCELED', 'date': '2018-09-12T21:27:25.241689'},
               {'id': 615064591, 'state': 'CANCELED', 'date': '2018-10-14T08:21:33.419441'}
              ]  
         →   →   [
                  {'id': 41428829, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364'},
                  {'id': 939719570, 'state': 'EXECUTED', 'date': '2018-06-30T02:08:58.425572'}
                 ]
    

**sort_by_date**

Сортирует список операций по дате (например по убыванию)

      Пример: [
               {'id': 41428829, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364'},
               {'id': 939719570, 'state': 'EXECUTED', 'date': '2018-06-30T02:08:58.425572'},
               {'id': 594226727, 'state': 'CANCELED', 'date': '2018-09-12T21:27:25.241689'},
               {'id': 615064591, 'state': 'CANCELED', 'date': '2018-10-14T08:21:33.419441'}
              ]
        →   →   [
                 {'id': 41428829, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364'}, 
                 {'id': 615064591, 'state': 'CANCELED', 'date': '2018-10-14T08:21:33.419441'}, 
                 {'id': 594226727, 'state': 'CANCELED', 'date': '2018-09-12T21:27:25.241689'}, 
                 {'id': 939719570, 'state': 'EXECUTED', 'date': '2018-06-30T02:08:58.425572'}
                ]
+ Модуль generators
модуль содержит новые функции,  генераторы для обработки данных.

**filter_by_currency**

Функция принимает на вход список словарей, представляющих транзакции.
Функция должна возвращает итератор, который поочередно выдает транзакции, 
где валюта операции соответствует заданной (например, USD).



      Пример: {
          "id": 939719570,
          "state": "EXECUTED",
          "date": "2018-06-30T02:08:58.425572",
          "operationAmount": {
              "amount": "9824.07",
              "currency": {
                  "name": "USD",
                  "code": "USD"
              }
          },
          "description": "Перевод организации",
          "from": "Счет 75106830613657916952",
          "to": "Счет 11776614605963066702"
      }
      {
              "id": 142264268,
              "state": "EXECUTED",
              "date": "2019-04-04T23:20:05.206878",
              "operationAmount": {
                  "amount": "79114.93",
                  "currency": {
                      "name": "USD",
                      "code": "USD"
                  }
              },
              "description": "Перевод со счета на счет",
              "from": "Счет 19708645243227258542",
              "to": "Счет 75651667383060284188"
       }

**transaction_descriptions**

генератор, который принимает список словарей с транзакциями
 и возвращает описание каждой операции по очереди.

      Пример:descriptions = transaction_descriptions(transactions)
     for _ in range(5):
    print(next(descriptions))

      Перевод организации
      Перевод со счета на счет
      Перевод со счета на счет
      Перевод с карты на карту
      Перевод организации

**card_number_generator**

 Генератор, который выдает номера банковских карт в формате 
XXXX XXXX XXXX XXXX, где X — цифра номера карты.
Генератор может сгенерировать номера карт в заданном диапазоне от 0000 0000 0000 0001 до 9999 9999 9999 9999.
Генератор, который принимает список словарей с транзакциями 

     Пример:for card_number in card_number_generator(1, 5):
    print(card_number)

    0000 0000 0000 0001
    0000 0000 0000 0002
    0000 0000 0000 0003
    0000 0000 0000 0004
    0000 0000 0000 0005


+ Модуль decorators
модуль для размещения декораторов.

**Декоратор log**
 автоматически логирует начало и конец выполнения функции, а также ее результаты или возникшие ошибки.
Декоратор принимает необязательный аргумент filename который определяет,
 куда будут записываться логи (в файл или в консоль): 
Если filename задан, логи записываются в указанный файл.
Если filename не задан, логи выводятся в консоль.

+ Модуль utils
модуль для использования данных, полученных из JSON-файла.

**load_transactions**
 Загружает транзакции из JSON-файла.
  Args:
        file_path: Путь к JSON-файлу

    Returns:
        Список словарей с данными о транзакциях

    Examples:
       >>> transactions = load_transactions('data/operations.json')
       >>> isinstance(transactions, list)
        True

+ Модуль external_api 
модуль для функции, которая принимает на вход транзакцию и возвращает сумму транзакции,
происходит обращение к внешнему API для получения текущего курса валют
и конвертации суммы операции в рубли.

 **convert_transaction_amount**
 Конвертирует сумму транзакции в рубли. 
  
 **get_exchange_rate**
  Получает текущий курс обмена валют через внешний API 
    
 **get_amount_from_transaction**
  Извлекает сумму из транзакции с учетом структуры operationAmount. 
    
 **get_currency_from_transaction**
 Извлекает код валюты из транзакции с учетом структуры operationAmount.
   
 **get_exchange_rate_with_cache**
 Получает курс с кэшированием на 5 минут. 
   
 **get_exchange_rate_with_retry**
 Получает курс с автоматическими повторными попытками при ошибках 
    
+ Модуль file_operations
модуль для функции, которая принимает источник данных о финансовых транзакциях в CSV- или XLSX-файлах. 
 **read_transactions_from_csv**
Считывает финансовые операции из CSV файла и возвращает список словарей.
Отрабатывает ошибки:
        FileNotFoundError: Если файл не найден
        pd.errors.EmptyDataError: Если файл пуст
        Exception: При других ошибках чтения файла

**read_transactions_from_excel**
Считывает финансовые операции из Excel файла и возвращает список словарей.
Отрабатывает ошибки:
        FileNotFoundError: Если файл не найден
        pd.errors.EmptyDataError: Если файл пуст
        Exception: При других ошибках чтения файла
 
+ Модуль process_operations
использует функции для работы с регулярными выражениями

**process_bank_search**
функция, принимает список словарей 
с данными о банковских операциях и строку поиска, а возвращает список словарей, 
у которых в описании есть данная строка.

**process_bank_operations**
функция, принимает список словарей с данными о банковских операциях и список категорий операций,
возвращает словарь, в котором ключи — это названия категорий, а значения — это количество операций в каждой категории.


## Добавление тестов.
**Функция get_mask_card_number**
Тестирование правильности маскирования номера карты.
и нестандартные длины номеров. Проверка работы функции на различных входных форматах номеров карт, 
включая граничные случаи. Проверка, что функция корректно обрабатывает входные строки, где отсутствует номер карты.

**Функция get_mask_account**
Тестирование правильности маскирования номера счета.
Проверка работы функции с различными форматами и длинами номеров счетов.
Проверка, что функция корректно обрабатывает входные данные, где номер счета меньше ожидаемой длины.

**Функция mask_account_card**
Тесты для проверки, что функция корректно распознает и применяет нужный тип маскировки в зависимости от типа
 входных данных (карта или счет). Параметризованные тесты с разными типами карт и счетов для проверки
 универсальности функции. Тестирование функции на обработку некорректных входных данных и проверка ее устойчивости
 к ошибкам.

**Функция get_date**
Тестирование правильности преобразования даты.
Проверка работы функции на различных входных форматах даты, включая граничные случаи и нестандартные строки с датами.
Проверка, что функция корректно обрабатывает входные строки, где отсутствует дата.

**Функция filter_by_state**
Тестирование фильтрации списка словарей по заданному статусу state
Проверка работы функции при отсутствии словарей с указанным статусом state в списке.
Параметризация тестов для различных возможных значений статуса state

**Функция sort_by_date**
Тестирование сортировки списка словарей по датам в порядке убывания и возрастания.
Проверка корректности сортировки при одинаковых датах.
Тесты на работу функции с некорректными или нестандартными форматами дат.
Общие аспекты тестирования

**Функция filter_by_currency**

Тестирование корректно фильтрует транзакции по заданной валюте.
Проверка правильно обрабатывает случаи, когда транзакции в заданной валюте отсутствуют.
Проверка на завершение ошибкой при обработке пустого списка или списка без соответствующих валютных операций.

**Функция transaction_descriptions**

Тестирование функции возвращает корректные описания для каждой транзакции.
Проверка правильно функции с различным количеством входных транзакций, включая пустой список.

**Функция card_number_generator**

Тестирование генератор выдает правильные номера карт в заданном диапазоне.
Проверка на корректность форматирования номеров карт.
Проверка, что генератор корректно обрабатывает крайние значения


**Декоратор log**

Тест успешного логирования в файл.
Тест логирования в консоль.
Тест логирования ошибки.
Тест множественных вызовов.
Тест логирования именованных аргументов.

**для функции load_transactions**

Подготовка перед каждым тестом.
Очистка после каждого теста.
Тест загрузки корректного JSON файла.
Тест пустого файла.
Тест отсутствующего файла.
Тест некорректного JSON.
Тест, когда JSON не является списком.
Тест пустого списка.
Тест ошибки доступа к файлу.
Тест файла с Unicode символами.
Тест обработки ошибок доступа к файлу.
Параметризованные тесты для load_transactions.

**для модуля external_api**
Тесты для вспомогательных функций извлечения данных.


Тест извлечения суммы из operationAmount.
Тест извлечения суммы из прямого поля amount.
Тест транзакции без суммы.
Тест транзакции с некорректной суммой.
Тест извлечения валюты из operationAmount.
Тест извлечения валюты из прямого поля currency.
Тест транзакции без валюты.
Тест извлечения валюты из словаря currency.
Тест успешного получения курса валют.
Тест получения курса для разных валют.
Тест ошибки API.
Тест ошибки соединения. 
Тест отсутствия API ключа.
Тест кэширования для разных валют.
и конвертации суммы операции в рубли.

 **convert_transaction_amount**
 Конвертирует сумму транзакции в рубли. 
  
 **get_exchange_rate**
  Получает текущий курс обмена валют через внешний API 
    
 **get_amount_from_transaction**
  Извлекает сумму из транзакции с учетом структуры operationAmount. 
    
 **get_currency_from_transaction**
 Извлекает код валюты из транзакции с учетом структуры operationAmount.
   
 **get_exchange_rate_with_cache**
 Получает курс с кэшированием на 5 минут. 
   
 **get_exchange_rate_with_retry**
 Получает курс с автоматическими повторными попытками при ошибках 
 
**тесты для модуля file_operations** 
 
**для функции read_transactions_from_csv**
Тест успешного чтения CSV файла.
Тест чтения пустого CSV файла.
Тест обработки ошибки при отсутствии файла.
Тест обработки ошибки пустого файла.
Тест обработки общей ошибки при чтении.

**для функции read_transactions_from_excel**
Тест успешного чтения Excel файла.
Тест чтения пустого Excel файла.
Тест обработки ошибки при отсутствии Excel файла.
Тест обработки ошибки пустого Excel файла.
Тест обработки общей ошибки при чтении Excel.

**тесты для модуля process_operations** 

**для функции process_bank_search**
Тест поиска точного совпадения.
Тест игнорирования регистра.
Тест поиска по части слова.
Тест поиск отсутствующей строки.
Тест на пустой поисковый запрос, должен возвращать все транзакции.
Тест на транзакции без ключа 'description' должны обрабатываться корректно.
Тест на поиск слов с юникод-символами

**process_bank_operations**
Тест на подсчет транзакций одной категории.
Тест на подсчет транзакций по нескольким категориям.
Тест на игнорирование регистра.
Тест на подсчет по частичному совпадению.
Тест на пустой список категорий, должен возвращать пустой словарь.
Тест на транзакции без description, не должны учитываться.
Тест на подсчет с специальными символами в категориях.



## Разработка:

+ Проект находится в активной разработке. План работы в дальнейшем:

+ Добавление новых модулей для расширения функционала

+ Улучшение обработки различных форматов данных



  
## Автор проекта:

Viktor Sapa 